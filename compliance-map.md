...
| Output VAT (TK 3331) on sales invoices | Luật Thuế GTGT số 48/2024/QH15, Điều 8.1 (10%) + Nghị quyết 204/2025/QH15 (8% through 2026-12-31) | `apps/api/src/sales/sales-invoice.service.ts` (effective-dated rate resolution → Dr 131 / Cr 511 / Cr 3331); `packages/db/src/seed.ts` (`tax_rules` rows `vat_rate` + `vat_rate_reduced`) | DONE |
| Input VAT (TK 1331) on purchase invoices + ≥VND 5,000,000 non-cash-payment credit condition | Luật Thuế GTGT số 48/2024/QH15 (input-VAT deduction; non-cash payment voucher required for invoices ≥ VND 5,000,000) + Nghị quyết 204/2025/QH15 (8% reduced rate through 2026-12-31) | `apps/api/src/purchasing/purchase-invoice.service.ts` (effective-dated rate resolution → Dr 156/152 / Dr 1331 / Cr 331; `nonCashPayment` flag recorded on `purchase_invoices`); weighted-average goods receipt via `apps/api/src/inventory/inventory.service.ts` (`applyReceipt`); `packages/db/src/seed.ts` (`tax_rules` rows `vat_rate` + `vat_rate_reduced`) | DONE |
| AR (TK 131) sub-ledger by customer — resolves dual-nature limitation | Thông tư 133/2016/TT-BTC (chi tiết công nợ phải thu theo từng khách hàng) | `apps/api/src/sales/ar.service.ts`; `journal_lines.partner_id` FK to `business_partners`; AR journal lines carry `partnerId` so per-customer debit/credit can be decomposed (resolves Phase-1 open-question on 131 dual-nature) | DONE |
| Weighted-average inventory valuation (TK 152 — nguyên liệu, TK 156 — hàng hóa) + COGS recognition (TK 632) on issue | VAS 02 — Hàng tồn kho; Thông tư 133/2016/TT-BTC, Điều 14 (phương pháp bình quân gia quyền liên hoàn) | `packages/domain/src/inventory.ts` (`receiptBalance`, `issueCost` — bigint-exact, no drift); `apps/api/src/inventory/inventory.service.ts` (`applyReceipt`, `applyIssue`); `packages/db/src/seed.ts` (Phase 2b demo movements) | DONE |
| AP (TK 331) sub-ledger by vendor — resolves dual-nature limitation | Thông tư 133/2016/TT-BTC (chi tiết công nợ phải trả theo từng nhà cung cấp) | `apps/api/src/purchasing/ap.service.ts`; `journal_lines.partner_id` FK to `business_partners` on every 331-account line; AP sub-ledger aggregates per-vendor debit/credit independently so vendor advances (debit balance on 331) are not masked by other vendors' payables. Reconciles to TB 331 aggregate. Resolves Phase-1 open-question on 331 dual-nature | DONE |
| E-invoicing issuance — selectable provider (Viettel/VNPT/MISA stubs) | Nghị định 123/2020/ND-CP; Thông tư 78/2021/TT-BTC; GDT XML schema 1450/QĐ-TCT; Nghị định 70/2025/ND-CP (amended) | `apps/api/src/einvoice/` (provider-stub registry, `EinvoiceService`, selectable via `companies.einvoice_provider`); `packages/db/src/schema/einvoices.ts`; serial uniqueness constraint (`einvoice_serial_uq`); at-most-one-issued constraint (`einvoice_one_issued_per_invoice`). **Live provider transmission not implemented — stubs only.** | PLANNED (stubs only; transmission Phase 2b+) |
